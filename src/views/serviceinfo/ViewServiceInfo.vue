            ...
            <div class="p-0 rounded-0">
                  ...
                  <div class="row no-gutters align-items-center">
                    ...
                    <div class="ml-5">
                      <button
                        ...
                        onclick="window.location.href='/create-serviceinfo'"
                      >Create Service Info</button>
                    </div>
                  </div>
                </div>
              ...
              <table class="mb-0 table table-hover table-striped">
                ...
                <tbody class="p-0">
                  ...
                  <tr class="table-body-contents" v-if="!allserviceinfos.length">
                  ...
                  <tr
                    ...
                    v-for="(serviceinfos,index) in allserviceinfos"
                    v-bind:key="serviceinfos.key"
                  >
                    <td>{{index + 1 + (currentPage - 1) * 10}}</td>
                    <td>{{ serviceinfos.title }}</td>
                    ...
                    <td class="pl-4">
                      <router-link
                        ...
                        v-b-tooltip.hover
                        title="Edit"
                        :to="{ name: 'Edit Serviceinfo', params: { _id: serviceinfos._id } }"
                        append
                      >
                      ...
                      <button
                        ...
                        v-b-modal.modal-1
                        @click="deleteData(serviceinfos._id)"
                      >
...
export default {
  ...
  created() {
    this.viewServiceInfos(this.currentPage);
  },

  methods: {
    deleteAndRefresh(obj) {
      service.deleteAll("ServiceInfo/deleteServiceInfo", obj, data => {
        this.viewServiceInfos(this.currentPage);
      });
    },
    viewServiceInfos(page) {
      this.currentPage = page;
      const formData = {};
      formData.page = page;
      formData.name = this.searchText;
      service.getAllServiceInfos(formData, data => {
        if (data.status === 200) {
          this.allserviceinfos = data.data.result;
