          ...
          <div class="card rounded-0">
            <div class="p-0 rounded-0">
                  ...
                  <div class="row no-gutters align-items-center">
                    <div class="col">
                      <input
                        ...
                        type="text"
                        v-model="searchText"
                        @input="viewServiceProcess(1)"
                        placeholder="Search"
                      />
                    </div>
                    <div class="ml-5">
                      <button
                        class="ml-auto font-weight-bold text-dark btn btn-warning"
                        onclick="window.location.href='/create-service-process'"
                      >Create Service Process</button>
                    </div>
                  </div>
                </div>
                <h5 class="card-title m-0">Service Process</h5>
              </div>
              <table class="mb-0 table table-hover table-striped">
                ...
                <tbody class="p-0">
                  <!-- <div class="text-center font-size-lg" v-if="!allserviceprocess.length">No data found.</div> -->
                  <tr class="table-body-contents" v-if="!allserviceprocess.length">
                  ...
                  <tr
                    ...
                    v-for="(serviceprocess,index) in allserviceprocess"
                    v-bind:key="serviceprocess.key"
                    :class="serviceprocess.bodyColor"
                  >View Servicing
                    <td>{{index + 1 + (currentPage - 1) * 10}}</td>
                    <td>{{ serviceprocess.title }}</td>
                    ...
                    <td class="pl-4">
                      ...
                      <router-link
                        ...
                        v-b-tooltip.hover
                        title="Edit"
                        :to="{ name: 'Edit ServiceProcess', params: { _id: serviceprocess._id } }"
                        append
                      >
                        <font-awesome-icon :icon="['fas', 'edit']" />
                      </router-link>
                      <button
                        ...
                        v-b-modal.modal-1
                        @click="deleteData(serviceprocess._id)"
                      >
...
export default {
  ...
  created() {
    this.viewServiceProcess(this.currentPage);
  },

  methods: {
    deleteAndRefresh(obj) {
      service.deleteAll("ServiceProcess/deleteServiceProcess", obj, data => {
        this.viewServiceProcess(this.currentPage);
      });
    },
    viewServiceProcess(page) {
      this.currentPage = page;
      const formData = {};
      formData.page = page;
      formData.name = this.searchText;
      service.getAllServiceProcess(formData, data => {
        if (data.status === 200) {
          this.allserviceprocess = data.data.result;
