          ...
          <div class="card rounded-0">
            <div class="p-0 rounded-0">
                  ...
                  <div class="row no-gutters align-items-center">
                    ...
                    <div class="ml-5">
                      <button
                        ...
                        onclick="window.location.href='/create-pricing'"
                      >Create Pricing</button>
                    </div>
                  </div>
                </div>
                <h5 class="card-title m-0">Pricing Structure</h5>
              </div>
              <table class="mb-0 table table-hover table-striped">
                ...
                <tbody class="p-0">
                  <div
                    class="text-center font-size-lg"
                    v-if="!allpricingstructure.length"
                  >No data found.</div>
                  <tr class="table-body-contents" v-if="!allpricingstructure">
                  ...
                  <tr
                    ...
                    v-for="(pricingstructure,index) in allpricingstructure"
                    v-bind:key="pricingstructure.key"
                  >
                    <td>{{index + 1 + (currentPage - 1) * 10}}</td>
                    <td>{{ pricingstructure.serviceName }}</td>
                    <td class="pl-4">
                      <router-link
                        class="text-warning btn px-1 py-0"
                        v-b-tooltip.hover
                        title="Edit"
                        :to="{ name: 'Edit Pricing', params: { _id: pricingstructure._id } }"
                        append
                      >
                      ...
                      <button
                        ...
                        v-b-modal.modal-1
                        @click="deleteData(pricingstructure._id)"
                      >
...
export default {
  ...
  created() {
    this.viewPricingStructure(this.currentPage);
  },

  methods: {
    deleteAndRefresh(obj) {
      service.deleteAll(
        "PricingStructure/deletePricingStructure",
        obj,
        data => {
          this.viewPricingStructure(this.currentPage);
        }
    ...
    viewPricingStructure(page) {
      this.currentPage = page;
      const formData = {};
      formData.page = page;
      formData.name = this.searchText;
      service.getAllPricingStructure(formData, data => {
        if (data.status === 200) {
          this.allpricingstructure = data.data.result;
    ...
    goToPage(page) {
      ...
      this.viewPricingStructure(page);
    }
  }
};
</script>
